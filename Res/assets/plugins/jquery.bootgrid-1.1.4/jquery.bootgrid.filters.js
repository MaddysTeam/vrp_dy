...
; (function ($, window, undefined) {
		...
		firstHeadRow.children().each(function () {
			var $this = $(this),
				 ...
				 column = {
				 	...
				 	headerCssClass: data.headercs || "", // change data.headerCssClass to data.headercs (leocaan)
	...
	*/
	...
	function renderActions() {
		if (this.options.navigation !== 0) {
			...
			if ((headerActions.length + footerActions.length) > 0) {
				...
				// Add show filters button. (leocaan)
				renderShowFilters.call(this, actions);
	...
	// Add show filters button. (leocaan)
	function renderShowFilters(actions) {
		var that = this,
				css = this.options.css,
				tpl = this.options.templates,
				filtersIcon = tpl.icon.resolve(getParams.call(this, { iconCss: css.iconFilters })),
				filters = $(tpl.actionShowFilters.resolve(getParams.call(this,
			   { content: filtersIcon, text: this.options.labels.filters })))
					.on("click" + namespace, function (e) {
						// todo: prevent multiple fast clicks (fast click detection)
						e.stopPropagation();
						switchShowFilters.call(that);
					});
		actions.append(filters);
	}

	// Add switch filters button. (leocaan)
	function switchShowFilters() {
		this.options.showFilters = !this.options.showFilters;
		this.options.showFiltersHandler(this.options.showFilters);
	}
	...
	function renderSearchField() {
		if (this.options.navigation !== 0) {
			...
			if ((headerSearch.length + footerSearch.length) > 0) {
				...
				// Add default value in search filed. (leocaan)
				searchField.val(this.options.defaultSearch);

				// Modify, change trige from 'keyup' to 'change'. (leocaan)
				searchField.on("change" + namespace, function (e) {
					e.stopPropagation();
					...
					if (currentValue !== newValue) {
						...
						}, 1000); // change 250ms to 1s (leocaan)
					}
				});
	...
	var Grid = function (element, options) {
		...
		this.rows = [];
		this.searchPhrase = options.defaultSearch;//""; // Add default value in search filed. (leocaan)
		this.selectedRows = [];
		this.sort = {};
		this.total = 0;
		this.totalPages = 0;
	...
	Grid.defaults = {
		...
		// Add flag what show filters. (leocaan)
		showFilters: false,
		...
		// Add the handler of show filters. (leocaan)
		...
		css: {
			...
			// add filters icon defined. (leocaan)
			iconFilters: "glyphicon-filter",
		...
		labels: {
			...
			refresh: "Refresh",
			// add the lable of filters. (leocaan)
			filters: "Filters",
			search: "Search"
		},
		...
		templates: {
			...
			// add the template of show filters. (leocaan)
			actionShowFilters: "<span class=\"btn btn-default\" type=\"button\" title=\"{{ctx.text}}\">{{ctx.content}}</span>",
